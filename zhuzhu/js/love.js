...
function Next() {
  ...
  block.style.left = block_left + 40 * offset_pint["block" + index][0] + "px";
  block.style.top = block_top - 40 * offset_pint["block" + index][1] + "px";
  for (let i = 0; i < block.children.length; i++) {
    // block.children[1].innerText = index;    // 编号便于调试
    block.children[i].style.left = blk_pint["block" + index][i][0] * -40 + "px";
    /* -40 是因为逻辑坐标和浏览器的x，y轴方向不一样*/
    block.children[i].style.top = blk_pint["block" + index][i][1] * -40 + "px";
  }
